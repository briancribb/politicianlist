...
function MemberList() {
  ...
  const getMembers = ()=>{
    let listItems = memberData.map((member, index)=>{
      ...
      return (
        <div key={`key_${member.id}`} className={'member card mb-3 border border-'+partyColor}>
          <div className={'card-header bg-'+partyColor+' text-white d-sm-flex justify-content-between'}>
            ...
            &nbsp;{member.partyName}</div>
            <div className="next">Start: <strong>{member.startYear}</strong></div>
          </div>
          <div className="card-body">
            <div className="d-sm-flex">
              <div className="member-photo-wrapper mb-2 mb-sm-0">
                <img className="member-photo img-fluid mx-auto ml-sm-0" onError={handleImageError} src={member.imageUrl || ""} loading="lazy" alt={`Photo of ${member.fullName}`} />
        ...
        <div className="row">
          <div className="col">
            <div className="btn-group btn-group-lg w-100 mb-3" role="group" aria-label="Modal launch buttons">
              <button type="button" className="d-inline-block btn btn-light flex-fill border border-dark w-50" data-view="filters" onClick={()=>{console.log("Clicked",this)}}>Filters</button>
              <button type="button" className="d-inline-block btn btn-light flex-fill border border-dark w-50" data-view="sorting" onClick={()=>{console.log("Clicked",this)}}>Sorting</button>
            </div>
          </div>
        </div>
        <div className="row">
          <div className="col">
            <input className="form-control form-control-lg" type="text" placeholder="Filter by name" onChange={()=>{console.log("Changed",this)}}/>
